...
if func == "req":
    ...
    basic_args = ("python", "-m", "pip", "install", "--upgrade")
    out = []
    pkg = []
    for req in requires:
        prefer_binary = "--prefer-binary" if "--prefer-binary" in req else ""
        require = req.replace("--prefer-binary", "").replace(" ", "")
